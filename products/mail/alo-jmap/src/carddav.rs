//! CardDAV (RFC 6352) over the contacts store — the protocol phones and
//! desktops (Apple Contacts, Thunderbird, DAVx5) sync against natively.
//!
//! One collection per account: `/dav/addressbooks/<user>/default/`, whose
//! address-object resources are `<contactId>.vcf`. Discovery is the
...
use alo_store::vcard;
use alo_store::{AccountStore, Contact, ContactId};
use axum::body::Body;
...
const NS: &str = "xmlns:d=\"DAV:\" xmlns:card=\"urn:ietf:params:xml:ns:carddav\" xmlns:cs=\"http://calendarserver.org/ns/\"";
...
fn options() -> Response {
    ...
    h.insert("DAV", header::HeaderValue::from_static("1, 3, addressbook"));
    h.insert(
        header::ALLOW,
...
async fn report(acc: &AccountStore, uid: &str, resource: &Resource, body: &[u8]) -> Response {
    if !matches!(resource, Resource::Addressbook) {
...
async fn get_object(acc: &AccountStore, resource: &Resource, head: bool) -> Response {
    let Resource::Object(id) = resource else {
        return status(StatusCode::NOT_FOUND);
    };
    match fetch(acc, id).await {
        Some(c) => {
            let body = vcard::to_vcard(&c);
            let etag = etag(&c);
...
async fn put_object(
    ...
    contact.id = ContactId::new(id.clone());
    match acc.put_contact(&ContactId::new(id.clone()), &contact).await {
        Ok(created) => {
            let etag = etag(&contact);
            let code = if created {
                StatusCode::CREATED
            } else {
...
async fn delete_object(acc: &AccountStore, resource: &Resource, headers: &HeaderMap) -> Response {
    ...
    match acc.delete_contact(&ContactId::new(id.clone())).await {
