//! The finance-report HTTP surface (alo Finance, ADR 0035, wave B4.11) — the
//! reports that are the journal added up, over [`alo_store::fin_pl`] and the
//! folds that follow it.
//!
//! The first of the four is the **profit and loss**: `GET /finance/reports/pl`
//! answers JSON for the screen and `GET /finance/reports/pl.csv` answers the
//! same figures as a file for the accountant — separate paths rather than one
//! route with a `?format=`, exactly as `/print` and `/pdf` are, because a URL
//! that names its representation is the one a browser saves under a sensible
//! name and a script quotes without a query string. Both call the same store
//! function, so the file and the screen cannot disagree about a cent.
//!
//! Four decisions this file makes rather than the store.
//!
//! - **The period is stated, never guessed.** Both `from` and `to` are
//!   required, both are plain days, and a missing or malformed one is a `422`.
//!   A report that quietly defaulted to "this quarter" would put a figure under
//!   a heading nobody asked for, which is the one thing a document copied into
//!   a year-end must not do. The *comparative* period is the store's
//!   ([`alo_store::comparative_period`]) and is returned rather than asked for.
//! - **Admin only, for now.** A P&L is the whole tenant's result, not the
//!   reader's own work: unlike `GET /finance/periods` (which any member may
//!   read, because knowing the books are shut stops them typing into them),
//!   this is the figure a company keeps to its officers. B4.12's accountant
//!   role widens the gate **additively** — a decision recorded in
...
mod tests {
    ...
    fn an_end_that_is_not_a_plain_day_is_refused_never_guessed_at() {
        for bad in [
            ...
            let problem = query(Some(bad), Some("2026-12-31"))
                .days()
                .err()
    ...
    fn a_well_formed_period_is_read_as_two_plain_days() {
        let (from, to) = query(Some("2026-01-01"), Some(" 2026-12-31 "))
            .days()
        ...
        assert!(query(Some("2026-12-31"), Some("2026-01-01")).days().is_ok());
    }
