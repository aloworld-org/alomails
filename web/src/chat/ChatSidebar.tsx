  ...
  return (
    <aside className="flex min-h-0 w-[24rem] shrink-0 flex-col border-r border-subtle bg-surface max-xl:w-80 max-md:w-full">
      <header className="flex min-h-16 items-center justify-between px-5">
        ...
        <span className="relative rounded-lg border border-subtle bg-surface"><IconButton size="sm" label={strings.chatCompose} icon={<Pencil size={16} />} onClick={() => setComposeOpen((open) => !open)} active={composeOpen} />{composeOpen && <span className="absolute right-0 top-full z-30 mt-2 flex min-w-44 flex-col rounded-xl border border-subtle bg-surface p-1.5 shadow-md"><button type="button" className="flex min-h-10 items-center gap-2 rounded-lg px-3 text-sm text-primary transition-colors hover:bg-accent-soft hover:text-accent" onClick={() => { setComposeOpen(false); props.onCreateChannel(); }}><Hash size={15} />{strings.chatNewChannel}</button><button type="button" className="flex min-h-10 items-center gap-2 rounded-lg px-3 text-sm text-primary transition-colors hover:bg-accent-soft hover:text-accent" onClick={() => { setComposeOpen(false); props.onStartDm(); }}><Users size={15} />{strings.chatNewDm}</button></span>}</span>
      </header>
  ...
  return (
    <li className="group relative flex items-center">
      <button type="button" aria-current={open ? "page" : undefined} style={rowStyle} className={`${rowClass} ${direct ? "!min-h-[4.5rem] py-3" : ""} ${open ? "font-medium" : ""} ${room.archivedAt !== null ? "opacity-60" : ""}`} onClick={onOpen}>
        {agent ? <span className="relative flex size-11 shrink-0 items-center justify-center rounded-xl bg-accent-tint text-accent"><Bot size={20} /><span className="absolute -bottom-1 -right-1 rounded border border-surface bg-surface px-1 text-[0.55rem] font-bold uppercase leading-4 text-accent">AI</span></span> : room.kind === "dm" ? <Avatar name={label} email={room.counterpart ?? undefined} size="lg" /> : room.visibility === "private" ? <Lock size={18} className={roomIconClass} /> : <Hash size={18} className={roomIconClass} />}
