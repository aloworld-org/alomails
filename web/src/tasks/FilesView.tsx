...
export function FilesView({ projectId, onOpen, onCreate }: Props) {
  ...
  return (
    ...
    >
      ...
      {files.map((f) => (
        <article key={f.id} className="group flex min-w-0 items-center gap-3 rounded-xl border border-subtle bg-surface p-3 shadow-sm transition-[border-color,box-shadow] hover:border-default hover:shadow-md">
