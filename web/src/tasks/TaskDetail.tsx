    ...
    <div className={styles.detailScrim} onMouseDown={onClose}>
      <div className={styles.detail} onMouseDown={(e) => e.stopPropagation()}>
        <div className={styles.detailHead}>
          ...
          <select value={t.status} onChange={(e) => void changeStatus(e.target.value)}>
            {COLUMNS.map((c) => (
        ...
        <div className={styles.detailBody}>
          ...
          <div className={styles.fieldRow}>
            <label className={styles.field}>
              <span className={styles.fieldLabel}>{strings.taskAssignee}</span>
              <input
            ...
            <label className={styles.field}>
              <span className={styles.fieldLabel}>{strings.taskDue}</span>
              <input
                type="date"
                defaultValue={dueDate}
                onChange={(e) =>
                  void save({ dueAt: e.target.value ? `${e.target.value}T12:00:00Z` : null })
                }
              />
            </label>
            <label className={styles.field}>
              <span className={styles.fieldLabel}>{strings.taskPriority}</span>
              <select value={t.priority} onChange={(e) => void save({ priority: e.target.value as TaskPriority })}>
          ...
          <label className={styles.field}>
            <span className={styles.fieldLabel}>{strings.taskDescription}</span>
            <textarea
              rows={3}
          ...
          <div className={styles.field}>
            <span className={styles.fieldLabel}>{strings.taskSubtasks}</span>
          ...
          <div className={styles.field}>
            <span className={styles.fieldLabel}>{strings.taskComments}</span>
            {data.comments.map((c) => (
          ...
          {data.activity.length > 0 && (
            <div className={styles.field}>
              <span className={styles.fieldLabel}>{strings.taskActivity}</span>
              {data.activity.slice(0, 12).map((a, i) => (
                <div key={i} className={styles.activity}>
                  {a.actor} · {strings.taskActivityKind(a.kind)} ·{" "}
