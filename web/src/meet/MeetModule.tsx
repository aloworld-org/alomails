      ...
      <div className={styles.content}>
        ...
        <div className={styles.dashboard}>
          <main className={styles.primary}>
            ...
            <section className={styles.meetings}>
              ...
              {loadProblem ? <div className={styles.state} role="alert"><AlertCircle /><strong>{strings.meetLoadFailed}</strong><Button variant="ghost" icon={<RefreshCw />} onClick={() => void load()}>{strings.meetRetry}</Button></div>
                ...
                : <ul className={styles.list}>{live.map((meeting) => <Card as="li" pad="sm" key={meeting.id} className={styles.meetingCard}><div className={styles.cardTop}><span className={styles.cardIcon}><Video /></span><span className={styles.livePill}><i />{strings.meetLive}</span></div><h3>{meeting.title.trim() || strings.meetUntitled}</h3><p>{meeting.startedAt === null ? strings.meetNotStarted : strings.meetStartedAt(time(meeting.startedAt))}</p><Button onClick={() => enter(meeting.id)}>{strings.meetJoin}<ArrowRight /></Button></Card>)}</ul>}
            </section>
          ...
          <aside className={styles.sidebar}>
            <Card as="section" className={styles.dateCard}><div><span>{month}</span><CalendarDays /></div><strong>{now.getDate()}</strong><p>{new Intl.DateTimeFormat(getLocale(), { weekday: "long" }).format(now)}</p></Card>
