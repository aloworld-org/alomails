...
const driveUploadBlob = vi.fn(async () => ({ id: "node-1", blobId: "blob-9", size: 3 }));
vi.mock("../jmap", () => ({
...
const PRESETS: ThemePreset[] = [
  {
    ...
    typography: { headingFamily: "system-ui", bodyFamily: "system-ui", headingWeight: 700 },
  },
  {
    id: "terra",
    name: "Terra",
    ...
    typography: { headingFamily: "Georgia", bodyFamily: "system-ui", headingWeight: 700 },
  },
];
...
async function openThemeDialogFromSiteView(theme: Record<string, unknown>) {
  ...
  replies.push(siteReply(theme), siteReply(theme), presetsReply());
  ui("/sites/site-1");
...
describe("the theme dialog", () => {
  test("renders the shipped presets and PUTs exactly the picked one", async () => {
    ...
    expect(screen.getByRole("radio", { name: "North" }).getAttribute("aria-checked")).toBe(
      "true",
    );
  ...
  test("an uploaded logo goes through Drive and lands in the envelope", async () => {
    await openThemeDialogFromSiteView({});
    fireEvent.click(screen.getAllByText(strings.sitesThemeUpload)[0] as HTMLElement);
    const file = new File(["png"], "logo.png", { type: "image/png" });
    const pickers = document.querySelectorAll('input[type="file"]');
    fireEvent.change(pickers[0] as HTMLInputElement, { target: { files: [file] } });
  ...
  test("a stored logo prefills; removing it drops the key from the envelope", async () => {
    ...
    expect(screen.getByRole("radio", { name: "Terra" }).getAttribute("aria-checked")).toBe(
      "true",
    );
  ...
  test("a refusal shows the server's sentence and the dialog stays open", async () => {
    await openThemeDialogFromSiteView({});
    replies.push({
      match: (url, method) => method === "PUT" && url.endsWith("/sites/site-1/theme"),
      status: 422,
...
describe("section image upload", () => {
  test("uploading in the hero form fills the blob id the section then saves", async () => {
    ...
    const section = (lastWrite()?.body as { section: { image?: { blob_id: string } } })
      .section;
