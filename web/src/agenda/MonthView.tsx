...
export function MonthView({ anchor, today, events, onDayClick, onEventClick }: Props) {
      ...
      <div className={styles.monthGrid}>
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={`${styles.dayCell} ${otherMonth ? styles.otherMonth : ""}`}
              ...
              <div className={styles.dayEvents}>
                {dayEvents.slice(0, MAX_CHIPS).map(({ e, s }) => (
                  <button
                    ...
                    title={e.summary}
                  >
                    {!e.allDay && <span className={styles.chipTime}>{timeFmt.format(s)}</span>}
