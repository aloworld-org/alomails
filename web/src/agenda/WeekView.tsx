...
import { HOURS_IN_DAY, eventOnDay, hourFraction, sameDay, startOfDay, weekDays } from "./dates";
...
export function WeekView({ anchor, today, events, onSlotClick, onEventClick }: Props) {
  ...
  const parsed = events.map((e) => ({ e, s: new Date(e.startsAt), en: new Date(e.endsAt) }));
        ...
        {days.map((d) => {
          ...
          return (
            <div key={d.toISOString()} className={styles.weekDayHead}>
              ...
              <span className={`${styles.weekDayNum} ${isToday ? styles.todayNum : ""}`}>
                {d.getDate()}
              </span>
            </div>
          );
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              ...
              {timed
                .filter(({ s, en }) => s.getTime() < dayEnd && en.getTime() > dayStart)
                .map(({ e, s, en }) => {
                  const top = Math.max(0, hourFraction(s)) * HOUR_HEIGHT;
                  const endFrac = en.getTime() >= dayEnd ? HOURS_IN_DAY : hourFraction(en);
                  ...
                  return (
                    <button
                      ...
                      title={e.summary}
                    >
                      <span className={styles.blockTitle}>{e.summary || "—"}</span>
                      <span className={styles.blockTime}>{timeFmt.format(s)}</span>
                    </button>
                  );
                })}
            </div>
