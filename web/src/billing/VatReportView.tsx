    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-8 pb-8 pt-6 max-sm:px-4">
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="m-0 text-xl font-semibold text-primary">
              {strings.billingReports}
            </h2>
            <p className="mt-1 max-w-4xl text-sm leading-relaxed text-secondary">
              {strings.billingReportBasis(
                readable(period.from),
                readable(period.to),
              )}
            </p>
          </div>
      ...
      {loading ? (
        ...
        <section className="flex min-h-72 flex-1 flex-col items-center justify-center rounded-xl border border-subtle bg-surface px-6 py-12 text-center shadow-sm">
          <span
            className="mb-4 flex size-14 items-center justify-center rounded-2xl bg-[var(--accent-soft)] text-accent"
            aria-hidden="true"
          >
            <CalendarRange className="size-6" />
          </span>
          <h2 className="m-0 text-lg font-semibold text-primary">
            {strings.billingReportEmptyTitle}
          </h2>
          <p className="mb-0 mt-2 max-w-lg text-sm leading-relaxed text-secondary">
            {strings.billingReportEmptyBody}
          </p>
        </section>
      ) : (
            ...
            <table className={tableClass}>
              ...
              <tfoot className="bg-raised text-primary">
                <tr>
                  <th
                    scope="row"
                    className="border-t border-default px-4 py-3 text-left font-semibold"
                  >
                    {strings.billingReportTotal}
                  </th>
                  <td
                    className={`border-t border-default px-4 py-3 ${numberCell}`}
                  >
        ...
        vatReportRestatesAnything(report) && (
          <section className={tableShell}>
            ...
            {report.base.unconvertedCount > 0 && (
              <ErrorBanner
                message={strings.billingReportUnconverted(
                  report.base.unconvertedCount,
                )}
              />
            )}
            <table className={tableClass}>
